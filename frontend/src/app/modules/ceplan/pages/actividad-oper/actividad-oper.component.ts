...
export class ActividadOperComponent implements OnInit {
  ...
  public setProgramado(data: any) {
    ...
    this.centro_costos_hsb=data[0].CENTRO_COSTO_HSB;
  }
  ...
  this.tipoEstadoR = tipo_registro === "BANCO DE DATOS" ||"SIGHOS" ? 'VALIDADO' : 'REGISTRADO';
  ...
  public cerrar_registro(fecha_cierre: any, tipo_registro: any) {
    ...
    const dia = String(fechaActual.getDate()).padStart(2, "0");
    const fecha_ac = `${año}-${mes}-${dia}`;
    if ((tipo_registro == "BANCO DE DATOS" || tipo_registro=="SIGHOS") || fecha_cierre < fecha_ac) {
      return true;
    } else {
      return false;
    }
